...
WelcomeScene.BT_WECHAT = 0
WelcomeScene.BT_CITY = 1
WelcomeScene.BT_REG = 2
...
function WelcomeScene:onCreate()
    ...
    -- 微信按钮
    local dy = 120
    self._wechatBtn = ccui.Button:create("btn_wechat_0.png","btn_wechat_1.png")
        :move(appdf.WIDTH / 3 - (appdf.WIDTH / 3 - 292), dy)
        :setTag(WelcomeScene.BT_WECHAT)
        :addTo(self)
        :addTouchEventListener(btcallback)
    -- 博城按钮
    self._cityBtn = ccui.Button:create("btn_city_0.png","btn_city_1.png")
        :move(appdf.WIDTH / 2, dy)
        :setTag(WelcomeScene.BT_CITY)
        :addTo(self)
        :addTouchEventListener(btcallback)
    -- 快速注册按钮
    self._regBtn = ccui.Button:create("btn_reg_0.png","btn_reg_1.png")
        :move(appdf.WIDTH * 2 / 3 + (appdf.WIDTH / 3 - 292), dy)
...
--按键点击
function WelcomeScene:onButtonClickedEvent(tag,ref)

    if tag == WelcomeScene.BT_WECHAT then
        g_NativeToast("点击 微信登录")
    elseif tag == WelcomeScene.BT_CITY then
        g_NativeToast("点击 博城账号")
    elseif tag == WelcomeScene.BT_REG then
        g_NativeToast("点击 快速注册")
    end
